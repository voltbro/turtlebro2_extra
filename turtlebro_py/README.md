...
```
После установки пакета на Ардуино робота надо залить прошивку управляющую светодиодной лентой из директории `ino` этого пакета.
...
ti.shutdown()  # освободить ресурсы ROS 2 (опционально, но желательно)
```
...
tb.call(myfunc) #при нажатии на кнопку 24, вызовется функция myfunc и выведет "Привет из моей функции"
```
